...
Check [preim fpart of (xpred1 0%MM)].
About preim_seq.
Search _ "partition".
Search _ bijective.
About partition.
...
(* try 0
Definition ffconst (T : finType) (T' : eqType) (f : {ffun T -> T'}) (A : {set T}) :=
  [exists x : T, [forall y in A, f x == f y]].
Lemma wlog2 :
  exists (f_l : nat) (f_alpha : complexR ^ f_l) (f_a : complexR ^ f_l) (f_c : complexR)
    (f_part : {set {set 'I_f_l}}),
  (0%N < f_l)%N /\ 
  (forall i : 'I_f_l, f_alpha i is_algebraic) /\
  (forall i : 'I_f_l, f_a i \is a Cint) /\
  (forall i : 'I_f_l, f_a i != 0) /\
  (partition f_part [set: 'I_f_l]) /\
  (forall A, A \in f_part -> ffconst f_a A) /\
  (forall A, A \in f_part -> {in A &, injective f_alpha}) /\
  (forall A, A \in f_part -> [fset (f_alpha x) | x in A]%fset \is a setZroots c) /\
...
(* Try 1 partition, mais assez lourd avec une notation
Lemma blablabla (T : finType) (f_part : {set {set T}}) i:
  (index (nth set0 (enum f_part) i) (enum f_part) < #|f_part|)%N.
Admitted.

Lemma wlog2 : 
  exists f_l, (0%N < f_l)%N /\ exists (f_beta : complexR ^ f_l),
     (forall i : 'I_f_l, f_beta i is_algebraic) /\
     exists f_part, partition f_part [set: 'I_f_l] /\
       (forall B, B \in f_part -> (finfun (tnth (in_tuple [seq f_beta x | x in B])))  \is a setZroots 0) /\
       exists (f_b : complexR ^ #|f_part|), 
         (forall j, f_b j != 0) /\ (forall j, f_b j \is a Cint) /\
         (Cexp_span (finfun (fun i : 'I_f_l => f_b (Ordinal (blablabla f_part i)   ))) f_beta == 0).
*)

(* Try 2
Lemma wlog2 : partition et définition partielle des a
  exists f_l (f_alpha : complexR ^ f_l) f_part
  (f_a : {ffun {set 'I_f_l} -> complexR}),
  (0%N < f_l)%N /\ (partition f_part [set: 'I_f_l]) /\ 
  (forall A : {set 'I_f_l}, A \in f_part -> f_a A != 0) /\
  (forall A : {set 'I_f_l}, A \in f_part -> f_a A \is a Cint) /\
  (forall i : 'I_f_l, f_alpha i is_algebraic) /\
  (forall A, A \in f_part -> (finfun (tnth (in_tuple [seq f_alpha x | x in A]))) \is a setZroots 0) /\
  (\sum_(A in f_part) Cexp_span [ffun _ => f_a A] f_alpha == 0).
*)

(* Try 3  non fini
Lemma wlog2 :
  exists f_l (f_alpha : complexR ^ f_l) (f_part : nat ^ f_l) (f_a : {fmap nat -> complexR}),
  (0%N < f_l)%N /\
  (forall i : 'I_f_l, f_alpha i is_algebraic) /\
  (forall j, j \in codom f_part -> f_a j \is a Cint) /\ 
  (forall j, j \in codom f_part -> f_a j != 0) /\
  (Cexp_span (finfun (f_a \o f_part)) f_alpha == 0).
*)


(* Try 4 : type dépendant -> lisible, mais moche
Lemma wlog2 :
  exists f_r (f_a : complexR ^ f_r) (f_t : nat ^ f_r) (f_c : complexR)
    (f_alpha : forall i : 'I_f_r, complexR ^ (f_t i)),
  (0%N < f_r)%N /\
  (forall i : 'I_f_r, f_a i != 0) /\
  (forall i : 'I_f_r, f_a i \is a Cint) /\
  (forall i : 'I_f_r, (f_alpha i) \is a setZroots f_c) /\
  (forall i : 'I_f_r, injective (f_alpha i)) /\
  (forall i : 'I_f_r, forall j : 'I_(f_t i), f_alpha i j is_algebraic) /\
  \sum_(i < f_r) Cexp_span [ffun _ => f_a i] (f_alpha i) == 0.
*)
...
Definition ta := finfun 
   ((fun m => (size (msupp ('m_m : {mpoly complexR[L.+1]})))) \o mord).

Lemma tuple_ta i : 
   size (msupp ('m_(mord i) : {mpoly complexR[L.+1]})) == (ta i).
Proof. by rewrite /ta ffunE /=. Qed.



Search _ "partition".
About partition.
About preim_partition.
...
Lemma blabla im : 
  (sum_gamma im) \is a setZroots 
      (c ^+ ((msize (mpoly_gamma (mfun_t im))).-1 * (ta im)).+1).
Proof.
set m := mfun_t im.
...
have [|] := (ltnP i (ta im).+1).
  rewrite ltnS leq_eqVlt => /orP [/eqP i_nn | ordi].
    ...
    set P := \prod_ _ _.
    have <- : (size P).-1 = (ta im).
      by rewrite size_prod_XsubC /index_enum -enumT size_enum_ord ffunE /=.
    have -> : P`_(size P).-1 = lead_coef P by [].
    by rewrite (monicP _) ?Cint1 ?/P ?rpred_prod // => j _; rewrite monicXsubC.
  have ordinn1 : (i < (ta im).+1)%N by apply: (leq_trans ordi (leqnSn _)).
  ...
  have P_sym : P \is symmetric.
    ...
    set t := map_tuple _ _.
    have -> /= : [tuple msym s t`_i0 | i0 < (ta im)] = map_tuple (msym s) t.
  ...
  suff Hle : (msize P <= ((msize (mpoly_gamma m)).-1 * (ta im)).+1)%N.
  ...
  have : m' \in (in_tuple (msupp ('s_((ta im), ((ta im) - i)) : {mpoly complexR[(ta im)]}))).
    by rewrite -eq_m' mem_tnth.
  rewrite mem_msupp_mesym /mechar => /andP[/eqP m'_mdeg /forallP m'_i].
  rewrite -[X in (_ * X)%N](subn0 (ta im)) mulnC -sum_nat_const_nat big_mkord.
  ...
  apply: (@leq_trans (\sum_(i0 < (ta im) | m' i0 != 0) (msize (mpoly_gamma m)).-1).+1%N).
...
Definition final_t := finfun ((fun i => (ta i)) \o (tnth (in_tuple reindex_seq))). 
