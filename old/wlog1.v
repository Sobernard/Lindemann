...
Lemma polyMin_coef0_neq0 (x : complexR) (H : algebraic x) :
  ((polyMin H)`_0 == 0) = (x == 0).
Proof.
rewrite /polyMin.
case : ((Sumbool.sumbool_of_bool (x != 0))) => [a | /eqP ->]; last first.
...
Lemma meval_comp_mpoly (n : nat) (R : comRingType) (k : nat) (lq : n.-tuple {mpoly R[k]})
      (h : 'I_k -> R) (p : {mpoly R[n]}) :
  (p \mPo lq).@[h] = p.@[tnth [tuple of (map (meval h) lq)]].
Proof.  
...
Lemma sym_fundamental_seqroots S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) P m p (l : m.-tuple T) :
  seqroots P = l ->
  P \is a polyOver kS -> p \is a mpolyOver m kS ->
  p \is symmetric -> (lead_coef P)^+ (msize p) * p.@[tnth l] \in kS.
Proof.
move=> eq_l P_over p_over p_sym.  
...
Lemma sym_fundamental_seqroots_empil S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) P n m p (l : m.-tuple T) :
  seqroots P = l ->
  P \is a polyOver kS -> p \is a (mpolyOver m (mpolyOver n kS)) ->
  p \is symmetric -> 
  (lead_coef P)^+ (msize p) *: p.@[tnth (map_tuple (@mpolyC n T) l)
                                       ] \is a mpolyOver n kS.
Proof.
move=> eq_l P_over p_over p_sym.  
move: (sym_fundamental_subring p_over p_sym) => [q /andP[/eqP eq_qp /andP[size_le q_over]]].
...
move: (seqroots_pred eq_l P_over j); set c := lead_coef _.
rewrite -(mpolyOverC n).
suff -> : ((c *: 's_(m, j.+1)).@[tnth l])%:MP_[n] =
          c%:MP_[n] * ('s_(m, j.+1))
          .@[tnth (map_tuple (mpolyC n (R:=T)) l)] by [].
...
by apply: eq_bigr => k _; rewrite !mevalX tnth_map.
Qed.
...
(* a generaliser *)
Variable r : n.-tuple complexR.
Hypothesis r_bigmax : bigmaxC 0 r = nth 0 r (Ordinal n_gt0).

Definition mnm_to_C := 
  (fun (m : 'X_{1.. n}) => \sum_(i < n) ((tnth r i) *+ m i)).

Lemma mnm_to_CE m : 
  mnm_to_C m = \sum_(i <- m2s m) (tnth r i).
Proof.
...
rewrite [X in lect _ X](tnth_nth 0) -r_bigmax (tnth_nth 0).
by apply: bigmaxc_lect; rewrite size_tuple.
Qed.

Lemma mnm_bigmaxc_lt p :
  forall m, mdeg m = p.+1 -> uniq r ->
...
apply: (@lect_lt_trans _ (\sum_(i < p) tnth r (Ordinal n_gt0) + tnth r (tnth tm ord_max))).
  rewrite lect_add2r; apply: lect_sum => i _.
  rewrite !(tnth_nth 0) -r_bigmax.
  by apply: bigmaxc_lect; rewrite size_tuple.
rewrite ltct_add2l !(tnth_nth 0) -r_bigmax.
