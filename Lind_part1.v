...
Definition sroot_a i := Croots (poly_a i).

Lemma a_in_sroot_a i : a i \in (sroot_a i).
Proof.
apply/CrootsP; first by apply: poly_a_neq0.
...
move/flatten_mapP => [j j_in] /(CrootsP (poly_a_neq0 _)). 
...
Proof.
rewrite /U /c.
apply: (sym_fundamental_Croots_empil b_cset R_overCint R_sym).
Qed.
...
have : (forall i : 'I_deg_U.+1, true ->
    lecif (alpha (tnth tm i)) (bmaxf alpha) (tnth tm i == index_bmaxf alpha)).
  by move=> i _; apply: (bmaxf_lecif alpha_inj).
by move/lecif_sum; rewrite Hsum; move/lecif_refl/forallP/(_ j)/implyP; apply.
Qed.
