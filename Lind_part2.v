...
From mathcomp Require Import perm finfun fingroup ssralg poly ssrnum polydiv.
From mathcomp Require Import mxpoly ssrint rat polyorder finmap order.
From SsrMultinomials
Require Import mpoly.
From Lindemann
Require Import archi Rstruct Cstruct prelim.
...
Definition L := (size ((Croots poly_gamma))).-1. 
Lemma size_gamma : size ((Croots poly_gamma)) == L.+1. 
Proof.
have /(CrootsP poly_gamma_neq0) := (poly_gamma_root ord0); rewrite /L.
...
Proof.
have /and3P[_ /Croots_separable H _] := svalP (polyMinseq alpha_algebraic).
...
have alpha_in i : alpha i \in Tuple size_gamma.
  by apply/(CrootsP poly_gamma_neq0) /poly_gamma_root.
...
Lemma gamma_fset_roots : [fset gamma i | i : 'I_L.+1]%fset \is a setZroots c.
Proof. 
rewrite fset_rootsE big_fset /= ?/gamma; last by apply: gamma_inj.
rewrite (big_ffun_ord _ _ (tnth (Tuple size_gamma)) _ (fun x => ('X  - x%:P))).
rewrite -(big_tuple _ _ (Tuple size_gamma) xpredT (fun x => ('X - x%:P))) /=.
by rewrite -Croots_poly poly_gamma_Cint.
Qed.
...
have := gamma_fset_roots; rewrite fset_rootsE mevalX.
...
have [|] := (ltnP i (ta m).+1).
  ...
  have H := (sym_fundamental_fset_roots_proper_wfset _ P_Over P_sym).
  move: gamma_fset_roots; rewrite fset_rootsE big_fset; last by apply/gamma_inj.
  move/H => {H} H //=.
...
have d_neq0 : d != 0 by rewrite expf_neq0 // c_neq0.
have [s /perm_eqP Hperm /allP Hall]:= (Croots_decomp_polyMin d_neq0 Hover).
...
apply/idP/idP => [x_in | y_in].
  rewrite (setZconj_Croots_proper x_in f_conj) seq_fsetE /polyMin.
  by rewrite -(polyMinZ_pi (regr_gamma_alg Hx)) -/(polyMin _) -conjOfE.
rewrite (setZconj_Croots_proper y_in f_conj) seq_fsetE -conjOfE. 
...
have S_eqi : S =i dzeta_n0.
  ...
  have x_sr : x \in Croots (polyMin (dzeta_n0_alg x_in)).
    by apply/CrootsP/polyMin_root/polyMin_neq0.
  exists (enum_fset (seq_fset (Croots (polyMin (dzeta_n0_alg x_in))))). 
    apply/mapP; exists (seq_fset (Croots (polyMin (dzeta_n0_alg x_in))))=> //.
    rewrite mem_undup; apply/mapP; exists x => //.
    case: (eqVneq _ _) => [H | ]; last by rewrite x_in.
    rewrite /polyMin.
    by congr (_ (Croots (map_poly _ _))); apply: polyMinZ_pi.
...
have Hperm : perm_eq S (enum_fset dzeta_n0).
  ...
  case: (boolP (y \in S)) => [y_in | ].
    ...
    congr (seq_fset (Croots (map_poly _ _))); apply/conjOf_polyMinZ.
...
have Hconj f (H : f \in S_undup) : f \is a setZconj 
  ...
  set P := polyMin _.
  suff -> : f = seq_fset (Croots P) by apply: setZconj_polyMin.
...
have Pf_msupp (f : 'I_L.+1 ^ l.+1) : injective f -> perm_eq (msupp (Pf f)) 
  ...
  rewrite filter_predT -(@eq_map _ _ (fun i : 'I_l.+1 => [:: U_( f i)%MM]));
          last first.
...
have Hf (f : 'I_L.+1 ^ l.+1) i : injective f -> 
  i \in codom f ->
  lecif ((mpoly_gamma U_(i)%MM).@[gamma]) 
  ...
  by apply: bmaxf_lecif => u v; rewrite !ffunE; move/gamma_inj/inj_f.
...
have Hprod (h : {ffun ('I_L.+1 ^ l.+1) -> 'I_l.+1}) : 
  ...
  lecif (mpoly_gamma (m_of h)).@[gamma] (mpoly_gamma mm).@[gamma] (m_of h == mm). 
  ...
  have /lecifP := (ihs h Hh).
  case: (boolP (_ == _)) => [/eqP -> _| ].
    rewrite eqm_add2r.
    apply/lecifP; move/lecifP: (Hf f (f (h f)) inj_f (codom_f f (h f))).
    by case: ifP => [/eqP -> // | _ H]; rewrite !mevalD ltc_add2r.
  move=> m_prod_neq Hlttc_prod.
  have /lecifP := (Hf f (f (h f)) inj_f (codom_f f (h f))).
  case: (boolP (_ == _)) => [/eqP -> _ | ].
    rewrite eqm_add2l (negbTE m_prod_neq) !mevalD.
    by apply/lecifP; rewrite ltc_add2l.
  move=> m_sumf_neq Hlttc_sumf; apply/lecifP; case: ifP => [/eqP H | ].
    by rewrite -!mpoly_gammaD H eq_refl.
  by move=> _; rewrite !mevalD; apply: ltc_add.
...
rewrite (bigD1 bmm) //= big1 ?addr0; last first.
  ...
  case: (boolP [exists f, (f \in pffun_on ord0 D Rt) && (m_of f == i)]).
    ...
    have /lecifP := (Hprod f f_in); rewrite eq_f i_neqm.
    by move/ltc_eqF; rewrite H.
...
have /lecifP := (Hprod h h_in).
case: (boolP (m_of h == mm)) => [hm_eq _ | _]; last first.
  by rewrite ltc_neqAle eqmpg eq_refl.
...
suff : ltc (mpoly_gamma (m_of h)).@[gamma] (mpoly_gamma mm).@[gamma].
  by move/eqP: hm_eq => ->; move/ltc_eqF; rewrite eq_refl.
rewrite /m_of /mm (bigD1 f) //= [X in ltc _ (_ X).@[gamma]](bigD1 f) //=.
rewrite !mpoly_gammaD !mevalD.
apply (@lec_lt_trans _ ((mpoly_gamma U_( (f (h f)))).@[gamma] + 
    (mpoly_gamma (\sum_(i : 'I_L.+1 ^l.+1| injectiveb i && (i != f)) mf i)).@[gamma])); first last.
  move: inj_f => /injectiveP inj_f.
  rewrite ltc_add2r (ltc_lecif (Hf f _ _ _)) //.
  ...
  by apply: codom_f.  
apply: lec_add; first by rewrite lecc.
apply/(big_rec2 (fun x y => lec (mpoly_gamma x).@[gamma] (mpoly_gamma y).@[gamma])).
  rewrite /mpoly_gamma (eq_bigr (fun i => 0)); last first.
    by move=> i _; rewrite mnmE mulr0n.
  by rewrite big1 // lecc.
move=> z x1 x2 /andP[inj_z z_neqf] H; rewrite !mpoly_gammaD !mevalD.
apply: lec_add => //.
apply: (lec_of_leif (Hf z (z (h z)) _ _)).
  by apply/injectiveP.
by apply: codom_f.
Qed.
...
Definition seq_conj := Croots_decomp_polyMin
...
by move: (Hall (f, x) x_in) => /=; move/setZconj_over/fset_roots_lead_coef.
Qed.
...
+ have /mapP[xs xs_in eq_xs] := xp_in.
  ...
  rewrite fset_rootsE; move/polyOverP => Hover.
