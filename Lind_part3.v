...
From Coquelicot Require Import Hierarchy RInt.
From mathcomp Require Import all_ssreflect.
From mathcomp Require Import perm ssralg poly ssrnum ssrint rat polydiv mxpoly.
From mathcomp Require Import intdiv polyorder finmap order complex.
From SsrMultinomials Require Import mpoly.
From Lindemann Require Import archi Rstruct Cstruct Canalysis prelim.
...
Import GRing.Theory Num.Theory complex ComplexField Archi.Theory Archi.Order.
...
Definition Me := max_norm (fun i => exp (Rmax 0 (Re (-alpha i)))).

Definition Mm := max_norm (fun i => max_norm (fun j => Mt i j)).

Definition MJi1 := Archi.bound (Ma * Mal * exp Mal * Me * (norm c) ^+ l.+1 * Mm).

Definition MJip := Archi.bound ((norm c) ^+ l.+1 * (Mal *+ 2) * Mm).
...
apply: (sym_fundamental_Croots_for_leq part_partition) => //.
+ by apply/KmZ_over.
...
apply: (sym_fundamental_Croots_for_leq part_partition) => //; first last.
...
have -> : ((Fp i)^`(m.+1)) = 0 by rewrite poly.derivn_poly0 ?Hs.
...
Proof.
have Hm (x y : R) : x * y = scal x y by rewrite /scal /= /mult /= RmultE.
rewrite Hm -RInt_scal. 
+ by apply: RInt_ext=> x Hineq; rewrite -mulrA normM Hm.
by apply: ex_CrInt_norm => x H; Scont.
Qed.

Lemma maj_Ic i j :
  RInt (fun x => norm (Cexp(x *: -alpha j)*((Fp i).[x *: alpha j])))
     0 1 <=
  RInt (fun x => norm ((RtoC (exp(Rmax 0 (Re (-alpha j))))) 
        * ((Fp i).[x *: alpha j]) )) 0 1.
Proof.
...
rewrite /Cexp /norm /= !mul0r !subr0 !addr0 !scalarZ /=.
rewrite !exprMn -!mulrDr !add0r mulr0 mul1r subr0 expr0n /= addr0.
set im := Im (- alpha i).
have Htrigo y: (cos y) ^+ 2 + (sin y) ^+ 2 = 1.
  rewrite addrC !exprS !expr0 !mulr1 -RplusE -!RmultE; exact: sin2_cos2.
rewrite !Htrigo !mulr1 /= !sqrtr_sqr.
rewrite !gtr0_norm; do? apply/RltP/exp_pos.
have : (Re (- alpha j) * x) <= (Rmax 0 (Re (- alpha j))).
  move: (num_real (Re (-alpha j))); rewrite realE.
  move=> /orP [Hpos| Hneg].
    apply: (@ler_trans _ (Re (-alpha j))); last by apply/RleP/Rmax_r.
    rewrite -[X in _<=X]mulr1.
    by apply: ler_wpmul2l=>//; apply: ltrW.
  apply/(@ler_trans _ (Re (-alpha j) *0)).
...
Lemma maj_Id i j :
  RInt (fun x =>
         norm ((RtoC (exp(Rmax 0 (Re (-alpha j))))) *
                (Fp i).[x *: alpha j] : complexR)) 0 1 =
  norm (exp(Rmax 0 (Re (-alpha j)))) *
  RInt (fun x => norm ((Fp i).[x *: alpha j])) 0 1.
Proof.
have Hm (x y : R) : x * y = scal x y by rewrite /scal /= /mult /= RmultE.
rewrite Hm -RInt_scal.
apply: RInt_ext => x Hineq; rewrite -Hm normM.
...
have Hm (x y : R) : x * y = scal x y by rewrite /scal /= /mult /= RmultE.
rewrite Hm -RInt_scal; last by apply: ex_CrInt_norm => x H; Scont.
apply/RleP; apply: RInt_le; first by apply/RleP; apply: ler01.
  ...
  apply: (ex_RInt_ext (fun x => norm ((RtoC (Mal *+ 2) ^+ p.-1) *
     ((T i) ^+ p).[x *: alpha j]))).
    move=> x Hineq; rewrite -Hm normM norm_exp. 
...
move=> x [/RltP/ltrW H0 /RltP/ltrW  H1]; apply/RleP; rewrite -Hm.
...
have /RleP := (norm_scal x (alpha j)); move/ler_trans; apply; rewrite RmultE.
...
have HM: M = RInt (fun y => M) 0 1.
  rewrite RInt_const /scal /= /mult /= RmultE /Rminus RoppE RplusE.
...
apply: (@ler_trans _
  ...
  move=> x [/RltP/ltrW H0 /RltP/ltrW H1]; rewrite hornerM normM -Hm. 
...
have Hz : zi <= zs.
  ...
  set u := complex.Im _.
  have := (sin2_cos2 u); rewrite /Rsqr !RmultE !RplusE addrC => -> /=.
  rewrite sqrtr1 mulr1 ger0_norm; last by apply/ltrW/RltP/exp_pos.
  have H1 : (Re (alpha j) <= norm (alpha j)).
