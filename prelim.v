...
From mathcomp
Require Import finfun separable fingroup vector finmap order.
From SsrMultinomials Require Import mpoly ssrcomplements.
From Lindemann Require Import archi Rstruct Cstruct.
...
Import GRing.Theory Num.Theory Archi.Theory Archi.Order complex.
...
rewrite (ordnat ordi) -enum_val_nth. 
by rewrite (nth_map (enum_rank_in x0_in x0)) ?size_enum_ord ?nth_ord_enum.
Qed.
...
rewrite comp_mpolyE; apply/dhomogP => m /msupp_sum_le; rewrite filter_predT.
...
have:= p_neq0; rewrite -msupp_eq0 -[msupp _]filter_predT -has_filter.
...
move=> i; rewrite mcoeff_msupp mcoeff_sym -mcoeff_msupp. 
by rewrite -[i in X in _ = X](mpermKV s) (mem_map (@mperm_inj _ _)).
Qed.
...
by rewrite (map_comp (cons^~ [::])) flatten_seq1 filter_predT map_id.
Qed.
...
Definition fset_roots (S : pred_class) c := [qualify a f : {fset T} |
      ((c *: \prod_(x <- enum_fset f) ('X - x%:P)) \is a polyOver S)].

Fact fset_roots_key S c : pred_key (@fset_roots S c). Proof. by []. Qed.
Canonical fset_roots_keyed S c := KeyedQualifier (@fset_roots_key S c).

Lemma fset_rootsE S c f :
  (f \is a fset_roots S c) = 
    ((c *: \prod_(x <- enum_fset f) ('X - x%:P)) \is a polyOver S).
Proof. by []. Qed.

Lemma fset_roots_Croots S (ringS : @subringPred T S) (kS : keyed_pred ringS)
    (P : {poly T}) : separable_poly P -> P \is a polyOver kS ->
    seq_fset (Croots P) \is a fset_roots kS (lead_coef P).
Proof.
move/Croots_separable => uniq_P; rewrite {1}[P]Croots_poly /fset_roots.
by rewrite -(eq_big_perm _ (uniq_perm_eq (enum_fset_uniq _) _ (seq_fsetE _))).
Qed.

Lemma fset_roots_lead_coef S (ringS : @subringPred T S) (kS : keyed_pred ringS)
    c l : l \is a fset_roots kS c -> c \in kS.
Proof.
...
Lemma fset_roots_inj_over S (ringS : @subringPred T S) (kS : keyed_pred ringS) 
  c m  (l : T ^ m) :
  injective l -> [fset (l i) | i : 'I_m]%fset \is a fset_roots kS c ->
  c *: \prod_(i < m) ('X - (l i)%:P) \is a polyOver kS.
Proof. by move => inj_l; rewrite fset_rootsE big_fset //=. Qed.

Lemma Croots_pred S (ringS : @subringPred T S) (kS : keyed_pred ringS) c m 
  (l : T ^ m) : 
  c *: \prod_(i < m) ('X - (l i)%:P) \is a polyOver kS-> 
...
Lemma sym_fundamental_fset_roots_proper_wfset S (ringS : @subringPred T S) 
...
by apply/Croots_pred.
Qed.

Lemma sym_fundamental_fset_roots_proper S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) c m p (l : T ^ m) : injective l ->
  [fset (l i) | i : 'I_m]%fset \is a fset_roots kS c -> p \is a mpolyOver m kS ->
  p \is symmetric -> c ^+ (msize p).-1 * p.@[l] \in kS.
Proof.
...
rewrite mpolyXE_id -[(msize p).-1](@subnK (\sum_(j < m) i j)); last first.
  ...
  by rewrite -mdegE msize_mdeg_lt ?i_msupp.
rewrite exprD -scalerA mevalZ.
apply: (rpredM (rpredX _ (fset_roots_lead_coef l_setroots))).
rewrite -prodrXr -scaler_prod rmorph_prod /=.
apply: rpred_prod => j _; rewrite -exprZn rmorphX /=.
apply: rpredX; rewrite mevalZ mevalX !tnth_map /= -mevalZ.
by apply/Croots_pred/fset_roots_inj_over.
Qed.

Lemma sym_fundamental_fset_roots S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) c m p (l : T ^ m) : injective l ->
  [fset (l i) | i : 'I_m]%fset \is a fset_roots kS c -> p \is a mpolyOver m kS ->
...
Lemma sym_fundamental_Croots_empil S (ringS : @subringPred T S) 
...
move: (Croots_pred l_over j); rewrite -(mpolyOverC n) /=.
...
Lemma sym_fundamental_Croots_empil_proper S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) c n m p (l : T ^ m) :
  injective l -> [fset (l i) | i : 'I_m]%fset \is a fset_roots kS c ->
  p \is a (mpolyOver m (mpolyOver n kS)) -> p \is symmetric -> 
...
rewrite exprD mpolyCM -scalerA mevalZ.
apply: rpredM.
  by rewrite mpolyOverC; apply: (rpredX _ (fset_roots_lead_coef l_setroots)).
rewrite rmorphX /= -prodrXr -scaler_prod rmorph_prod /=.
apply: rpred_prod => j _; rewrite -exprZn rmorphX /=.
apply: rpredX; rewrite mevalZ mevalX /t !tnth_map /= tnth_ord_tuple.
move: (Croots_pred (fset_roots_inj_over l_inj l_setroots) j); rewrite -(mpolyOverC n).
suff -> : ((c *: 's_(m, j.+1)).@[l])%:MP_[n] =
          c%:MP_[n] * ('s_(m, j.+1))
          .@[finfun((mpolyC n (R:=T)) \o l)] by [].
rewrite -mul_mpolyC mevalM mpolyCM mevalC; congr (c%:MP_[n] * _).
...
Lemma sym_fundamental_Croots_mmsym_for S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) (c : T) m (l : T ^ m.+1) (A : {set 'I_m.+1}) 
  (md : 'X_{1..m.+1}) :
  injective l -> [fset l i | i in A]%fset \is a fset_roots kS c -> 
...
suff -> : ((\sum_(i < #|A|) md (enum_val i)) = (msize q).-1)%N.
  apply: sym_fundamental_fset_roots_proper.
...
Lemma sym_fundamental_Croots_for S (ringS : @subringPred T S) 
  (kS : keyed_pred ringS) c m p (l : T ^ m.+1) P :
  partition P [set: 'I_m.+1] -> injective l ->
  {in P, forall Q : {set 'I_m.+1}, 
    [fset l i | i in Q]%fset \is a fset_roots kS c} ->
...
have c_over : c \in kS.
  ...
  by apply: (fset_roots_lead_coef (l_set Q Q_in)).
...
move: p_over p_symm; wlog: p / (p \is homog [measure of mdeg]).  
  ...
  + move=> Q Q_in; rewrite fset_rootsE scale1r.
...
elim: n.
  ...
  have {Q_set} Q_set : [fset l i | i in Q]%fset \is a fset_roots kS 1.
  ...
  by apply: sym_fundamental_Croots_mmsym_for.
move=> n ihn P card_P.
...
have l'_set : {in P', forall Q : {set 'I_m.+1}, 
          [fset l i | i in Q]%fset \is a fset_roots kS 1}.
...
apply: H_ihn.
  ...
  apply: sym_fundamental_Croots_mmsym_for => //.
...
by rewrite -(permK s i) (out_perm (perm_onI s_on) i_notR) i_notR i_notQ.
Qed.

Lemma sym_fundamental_Croots_for_leq S (ringS : @subringPred T S) 
...
have c_in : c \in kS.
  ...
  have {HQ} Q_in : Q \in P by move: HQ; rewrite /(enum _) mem_filter => /andP[].
  by have := (fset_roots_lead_coef (H3 Q Q_in)).
have := (leq_sub2r 1%N k_leq); rewrite !subn1 /=.
move/subnKC => <-; rewrite exprD mulrAC rpredM ?rpredX //.
by apply: (sym_fundamental_Croots_for H1 H2 H3 H4 H5).
Qed.
...
Lemma Croots_polyMin x (H : x is_algebraic) :
  uniq (Croots (polyMin H)).
Proof. by rewrite Croots_separable ?polyMin_separable. Qed.
...
Lemma setZconj_over (c : complexR) (f : {fset complexR}) :
  f \is a setZconj c -> f \is a fset_roots (@Cint complexR_archiNumDomainType) c.
Proof.
rewrite setZconjE.
set P := c *: \prod_(x <- enum_fset f) ('X - x%:P).
case: (eqVneq (P != 0)) => H1 //.
...
Lemma setZconj_polyMin (x : complexR) (x_alg : x is_algebraic) :
  ...
  seq_fset (Croots P) \is a setZconj (lead_coef P).
Proof.
set P := polyMin x_alg; rewrite /=.
have H : perm_eq (enum_fset (seq_fset (Croots P))) (Croots P).
  apply: uniq_perm_eq; rewrite ?enum_fset_uniq ?Croots_polyMin //.
  by move=> y; rewrite seq_fsetE.
apply: (@setZconj_find _ _ x).
+ rewrite seq_fsetE.
  by apply/CrootsP; rewrite ?polyMin_neq0 ?polyMin_root.
+ by rewrite (eq_big_perm _ H) /= -Croots_poly polyMin_over.
+ rewrite (eq_big_perm _ H) /= -Croots_poly.
  by apply/(@root_size_gt1 _ x)/polyMin_root/polyMin_neq0.
apply/allP=> y; rewrite -/P (perm_eq_mem H) => /CrootsP.
...
Lemma setZconj_Croots (c : complexR) (f : {fset complexR}) 
  (Hf : f \is a setZconj c) :
  let p_neq0 := setZconj_neq0 Hf in
  let p_over := setZconj_over Hf in
  let x_root := svalP (sigW (closed_rootP _ (setZconj_size Hf))) in 
    f = seq_fset (Croots (polyMin (poly_algebraic p_neq0 x_root p_over))).
Proof.
...
apply/CrootsP; first by apply/polyMin_neq0.
by move: x_in; rewrite {1}(setZconj_Croots Hf) -/Py seq_fsetE.
Qed.

Lemma setZconj_Croots_proper (c x : complexR) (f : {fset complexR}) 
  (x_in : x \in f) (Hf : f \is a setZconj c) :
    f = seq_fset (Croots (polyMin (setZconj_algebraic x_in Hf))).
Proof.
...
Definition conjOf (x : complexR) (x_alg : x is_algebraic) :=
  [qualify a y | y \in (Croots (polyMin x_alg))].
...
Lemma conjOf_refl (x : complexR) (x_alg : x is_algebraic) :
  x \is a conjOf x_alg.
Proof. by apply/CrootsP/polyMin_root/polyMin_neq0. Qed.
...
apply/idP/idP; move/CrootsP; move/(_ (polyMin_neq0 _)) => H.
  apply/CrootsP; first by apply/polyMin_neq0.
...
have := y_in; rewrite (setZconj_Croots_proper xin Hf) seq_fsetE.
by rewrite /polyMin polyMinZ_pi.
Qed.

Lemma Croots_decomp_polyMin (a : seq complexR) (c : complexR) :
...
have Hperm : perm_eq (Croots (map_poly ZtoC pZ)) (mask ma a).
  rewrite eq_P CrootsZ; last by rewrite lead_coef_eq0 polyMin_neq0.
  apply/(perm_eq_trans (Croots_prod _)).
    by apply/allP => y _ /=; rewrite polyXsubC_eq0.
  rewrite (@eq_map _ _ _ (fun i => [:: i])); last first.
    by move=> i; rewrite Croots_XsubC.
  by rewrite flatten_seq1.  
have {Hperm} uniq_ma : uniq (mask ma a). 
  by rewrite -(perm_eq_uniq Hperm) Croots_polyMin.
...
Lemma Croots_decomp_polyMin_fset (a : {fset complexR}) (c : complexR) (o : a) :
...
have [s Hperm /allP Hall] := (Croots_decomp_polyMin H1 H2).
...
exists (map (fun x => (f x.1, x.2)) s).
  ...
  have Hl : map val (enum {: a}) = l.
    ...
    by congr undup; rewrite filter_predT.
